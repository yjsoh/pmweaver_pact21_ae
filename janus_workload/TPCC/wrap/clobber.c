...
POBJ_LAYOUT_BEGIN(TPCC);
POBJ_LAYOUT_ROOT(TPCC, TPCC_DB);
POBJ_LAYOUT_TOID(TPCC, BinaryData);
POBJ_LAYOUT_TOID(TPCC, int);
POBJ_LAYOUT_END(TPCC);

static PMEMobjpool *pop = NULL;
static TPCC_DB *popRoot = NULL;
int pertx_counter = 2;
...
void* init_runtime() {
    ...
    if (pop == NULL) {
        ...
        PMEMoid root = pmemobj_root(pop, sizeof(TPCC_DB));
        popRoot = D_RW((TOID(TPCC_DB))root);
    }
    assert(pop != NULL);
...
void* pmem_alloc(size_t size) {
    if (popRoot == NULL && sizeof(TPCC_DB) == size) {
        debug("%s\n", "allocating root");
        PMEMoid root = pmemobj_root(pop, sizeof(TPCC_DB));
        debug("%s: (0x%" PRIx64 ", 0x%" PRIx64 ")\n", "root", root.pool_uuid_lo, root.off);
        TPCC_DB *rootPtr = D_RW((TOID(TPCC_DB))root);
