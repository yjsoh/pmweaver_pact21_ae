...
POBJ_LAYOUT_BEGIN(TATP);
POBJ_LAYOUT_ROOT(TATP, TATP_DB);
POBJ_LAYOUT_TOID(TATP, BinaryData);
POBJ_LAYOUT_TOID(TATP, int);
POBJ_LAYOUT_END(TATP);

static PMEMobjpool *pop = NULL;
static TATP_DB *popRoot = NULL;
int pertx_counter = 2;
...
void* init_runtime() {
    ...
    if (pop == NULL) {
        ...
        PMEMoid root = pmemobj_root(pop, sizeof(TATP_DB));
        popRoot = D_RW((TOID(TATP_DB))root);
    }
    assert(pop != NULL);
...
void* pmem_alloc(size_t size) {
    if (popRoot == NULL && sizeof(TATP_DB) == size) {
        debug("%s\n", "allocating root");
        PMEMoid root = pmemobj_root(pop, sizeof(TATP_DB));
        debug("%s: (0x%" PRIx64 ", 0x%" PRIx64 ")\n", "root", root.pool_uuid_lo, root.off);
        TATP_DB *rootPtr = D_RW((TOID(TATP_DB))root);
