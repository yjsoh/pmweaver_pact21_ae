...
void *init_runtime()
{
    ...
    { // recover existing data structure
...
status_t PersistentTreeDestroy(BPTree **tree)
{
    ...
    double undonum = (double)(write + war) / tx;
    double undosize = (double)(writesize + warsize) / tx;
    double warnum = (double)(war) / tx;
    double warlogsize = (double)(warsize) / tx;
    double vlogsize = (double)vsize / tx;

    fprintf(stdout, "nolog ");
    fprintf(stdout, "count: 0, size: 0\n");

    fprintf(stdout, "undo ");
    fprintf(stdout, "count: %lf, size: %lf\n", undonum, undosize);

    fprintf(stdout, "vlog ");
    fprintf(stdout, "count: 1, size: %lf\n", vlogsize);

    fprintf(stdout, "warlog ");
    fprintf(stdout, "count: %lf, size: %lf\n", warnum, warlogsize);

    fprintf(stdout, "clobber ");
    fprintf(stdout, "count: %lf, size: %lf\n", warnum + 1, warlogsize + vlogsize);
